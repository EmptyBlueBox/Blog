...
The corner net comes first, followed by the edge net. U, L, F, R, B, and D identify the faces. The bold outlines mark this tutorial’s C buffers.

#### Speffz

<TutorialImages columns={2} images={[
  {"src":"https://cdn.lyt0112.com/2026/09/cubing/blindfold-speffz-corners-dd83cd15a793.svg","alt":"Speffz corner lettering","width":820,"height":620},
  {"src":"https://cdn.lyt0112.com/2026/09/cubing/blindfold-speffz-edges-c92cae04bcbd.svg","alt":"Speffz edge lettering","width":820,"height":620},
]} />
...
The usual fix is simple: append `B` to both the edge memo and the corner memo, then finish the solve with one `PLL Jb Perm: R U R' F' R U R' U' R' F R2 U' R'`.
...
The corner memo is `BO N`. If you still append `B`, the last pair becomes `NB`. Those two letters sit on the same corner, so they cannot be used as a legal target pair. In that case, append `A` instead. The corner memo becomes `BO NA`, and you finish with `PLL V Perm: U2 (R' U R U' R' f' U' R U2 R' U' R U' R' f R) U2`.
...
When both the edge memo and the corner memo have odd length, you need one extra parity algorithm at the end. Following the above description of adding a B at the end of the encoding, you need to additionally execute the `PLL Jb Perm: R U R' F' R U R' U' R' F R2 U' R'` algorithm for parity correction. This algorithm swaps the corner-edge buffer piece and the two B pieces at the same time, thereby changing the parity of the encoding.

Of course, if, as mentioned in the special case above, the last encoded piece and the added B fall on the same piece, then you need to add a different letter instead, such as A, and at the same time replace the final PLL with the corresponding algorithm, for example `PLL V Perm: U2 (R' U R U' R' f' U' R U2 R' U' R U' R' f R) U2`.

## Case Study: A Full Memo Example

Consider the following scramble with white on top and green in front, and yellow on top blue in front as memo coordinates: `D L' F2 R B2 D' B2 R' U' F' U' R2 D' B2 U' L U L F' R' D R2 D' B2`
...
Choose an unlettered piece as a new starting point. If you choose `A`, the piece at `A` is `W`, the piece at `W` is `R`, and the piece at `R` sends you back to the start of that small cycle. The cycle memo is now `AW R`, so you still need another cycle break to finish the trace.
...
Parity: execute `PLL Jb Perm: R U R' F' R U R' U' R' F R2 U' R'`.
