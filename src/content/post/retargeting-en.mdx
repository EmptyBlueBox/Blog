...
Hand motion retargeting maps the source hand's joint configuration (qpos) to an executable motion configuration on the target hand. The most direct idea is to copy joint rotations one by one, but this naive approach fails as soon as the two hands differ in degrees of freedom, link lengths, or joint limits, and the semantics of contact will drift.
