...
##### 1.5.2.1 使用管程的解决方案 (避免饥饿)
为了避免饥饿，我们需要引入一个更公平的调度策略。一种常见的策略是"先来先服务"或使用"闸门"来阻止新来的狒狒插队。
...
**3. 说明**
*   **公平性**: `turn` 和 `waiting` 计数器是实现公平的关键。当一个东行狒狒到达时，如果当前有西行狒狒在绳上，它必须等待。此外，即使绳子是空的，如果 `turn` 是 `WEST` 并且有西行狒狒 (`west_waiting > 0`) 已经在等待，那么新来的东行狒狒也必须等待，以确保先来的西行狒狒优先。
*   **切换**: 当一个方向的最后一头狒狒离开绳索时，它会将 `turn` 切换到另一个方向，并用 `broadcast` 唤醒所有等待的对向狒狒，从而实现了方向的轮转，避免了饥饿。
...
*   **条件变量**:
    *   `barber_available`: 理发师在此等待顾客。
    *   `customer_ready`: 顾客在此等待理发师叫号。
*   **管程过程**:
    *   `GetHaircut()`: 由顾客调用。
    *   `NextCustomer()`: 由理发师调用。
...
monitor BarberShop {
    int waiting_count = 0;
    final int N = 5; // 椅子数量
    condition barber_available, customer_ready;
