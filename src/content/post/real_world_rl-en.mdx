...
The technical report of $\pi_{0.6}^*$ describes a staged training recipe: offline pretraining, supervised fine-tuning, and online correction-driven RL. [^7]
...
- First they run offline RL on $\pi_{0.6}$, so the model learns action preferences from offline data
