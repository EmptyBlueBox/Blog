...
大多数时候只要引入 human demonstration, retargeting 就不可避免, 尤其是人类示教的模仿学习中.

纯几何映射无法处理复杂交互, 因此研究开始引入视觉和触觉信息. 但除了简单抓取, 大多数任务仍难以稳定迁移. 核心问题在于: 1) 不同物体需要不同的机器人配置, 需要物体几何作为条件; 2) 即使同一物体, 不同力分布也需要不同的目标姿态, 需要接触力作为条件.

理想情况下, 未来的强化学习可以直接学会灵巧操作, 人手动作只需作为输入对齐即可. 但目前这样的控制器还不存在, 因此基于物体与受力的交互感知重定向仍是最可行的方案.
...
在人手上就是指把人手或者机械手的动作转移到另一个手同时保持能够操控同样物体的一致性, 比如给定人手拿起一个木块的动作, 能够通过 retargeting 将动作转移给机械手, 最终让它也拿起这个木块. 所以这项能力是 teleoperation, imitation learning 和数据扩增的关键枢纽, 因为它让已有的人类示教得以跨本体复用.
