...
运动重定向 (Motion Retargeting) 是角色动画/机器人领域的一个概念, 指在把个体的关节状态 qpos 转换成另一个类似的个体的关节状态. 在人手上就是指把人手/机械手的动作转移到另一个机械手同时保持能够操控同样物体的一致性. 如果是同样的自由度, 那么最直接的想法是逐关节复制旋转角度, 但当两只手在自由度, 连杆长度和关节约束上不一致时, 这个 naive 方式马上失效, 物体接触的位置也会漂移.

这项能力是 teleoperation, imitation learning 和数据扩增的关键枢纽, 因为它让已有的人类示教得以跨本体复用. 真正的挑战在于人手与机器人手存在系统性的形态差异, 同时还要面对与物体的复杂接触. 纯几何角度映射难以保留交互语义和任务目标, 于是研究逐渐从几何匹配走向交互感知, 把物体形状, 受力, 触觉和动作意图纳入同一个学习闭环, 并积极采用自监督和无配对数据.
...
在实际操作中, 施加的力决定抓握稳定性和任务成败. 在我看来受力一致性几乎是决定 retargeting for interaction 成功与否的唯一指标. 即便物体形状相同, 施力差异也会引发物体目标姿态的显著变化.
...
这一方向试图摆脱人工配对数据, 直接从动作准则或共享潜空间中学习跨手映射.
...
- **未配对域转换**: 编码器解码器结构在不同关节域之间执行无监督翻译, 给定动作准则即可约束映射, 与 Geometric Retargeting 的自监督思路相呼应.

## 结论

只要引入 human demonstration, retargeting 就不可避免.
