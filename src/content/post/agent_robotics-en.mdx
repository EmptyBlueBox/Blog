...
Still, these demos are fundamentally quite similar. They mostly come down to pick and place (pnp): grab something, move it somewhere else, and put it down. The tasks have many names, but what the robot does is still quasi-static motion planning, meaning that over any short interval, the forces on the object are approximately balanced.

## Why Quasi-Static Motion?

Why do these tasks work? To put bread in a basket, the robot finds the bread, moves the gripper over, picks it up, moves above the basket, and lets go. Each step has a clear meaning and corresponds to a state that is relatively easy to judge from an image. Most of the time, the environment can even stay still while the model thinks.
...
This is a welcome change. A general model that understands different tasks and adapts to different interfaces means that improvements in the model could carry directly across robot embodiments[^5]. But these demos make me think that if VLAs cannot handle contact-rich manipulation tasks, they will ultimately fall short of GPT-6 Astra on open-vocabulary tasks[^12]. The low-level actions that are hard to describe in language are the most important part of robotics research.
